...
**One caveat, so the export's value is not overstated.** End to end on the live project, a warm hybrid query measured 567 ms with the check and 583 ms with it skipped — five runs each, and the second loop was the slower one, so the machine's own variance is larger than the 9.69 ms at stake. At this corpus size the flag does not make searches feel faster, and that is not what it is claimed for. It matters because the check is the one term in a query whose cost grows with the number of source files: at 1,000 sources it is roughly 176 ms, and at that point it is no longer invisible. The honest summary is that the flag removes a scaling term, not a current latency problem.
