...
- **Work around PyMuPDF's page-label crash until upstream fixes it.** `Page.get_label()` filters the document's label tree and indexes the result, so a document whose labels begin on page 2 raises `IndexError` on page 1 and fails the whole extraction, which is how a corpus with one such PDF cannot be ingested at all. `_pdf_locator` catches it and falls back to the physical page number. Narrow that guard once `pymupdf.utils.get_label_pno` stops indexing an empty list; reproduce by saving a two-page document with `set_page_labels([{"startpage": 1, "prefix": "", "firstpagenum": 1, "style": "D"}])` and calling `get_label()` on its first page.
