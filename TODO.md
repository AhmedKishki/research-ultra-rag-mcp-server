...
- **`--set` on the two surfaces that spawn a server.** `research-ultra-rag` resolves settings in its own process, so `--set` reaches every operation, including `ingest` and `search`. `research-ultra-rag-verify` and the browser UI still spawn a stdio server and cannot: `research-ultra-rag-verify --set retrieval.rrf_k=30 --ingest` reports `unchanged`, because the child resolves its own settings from the project, the environment, and the defaults, and `--set` overrode only the parent. An experiment driven that way silently measures the defaults. Environment variables (`RESEARCH_ULTRARAG_*`) do propagate, which is what the recorded measurements used. Either forward the parent's overrides into the child environment or say in the settings documentation which channel each entry point honours.
...
- **Work around PyMuPDF's page-label crash until upstream fixes it.** `Page.get_label()` filters the document's label tree and indexes the result, so a document whose labels begin on page 2 raises `IndexError` on page 1 and fails the whole extraction, which is how a corpus with one such PDF cannot be ingested at all. `_pdf_locator` catches it and falls back to the physical page number. Reported upstream as https://github.com/pymupdf/PyMuPDF/issues/5140, which carries the minimal reproduction: a two-page document built with `set_page_labels([{"startpage": 1, "prefix": "", "firstpagenum": 1, "style": "D"}])` raises on its first page. Narrow the guard when that issue closes.
