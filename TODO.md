...
- [ ] **Section-aware chunking, with the section as scope rather than as the answer.** Feature, the largest change here. Measured on the reference corpus: only 6 of 81 sources are EPUB and 12.3% of returned passages carry a section title, because a PDF locator is a page; and the 29 named sections hold a median of 7,491 words, so returning the enclosure would put about 10,000 tokens behind every hit where a passage is 200. What the measurement does support is the section as a *boundary for chunking*: chunks never span extraction units, so Crawford's 3,424 units produce 3,426 chunks and its index becomes 783 fragments, and 16.6% of the corpus's chunks are under twelve words. Packing units up to `chunking.size` within a section would take the corpus from 19,400 chunks to roughly 8,150, delete the fragment class at its source, and give the dense index fewer and longer vectors; only then is the section worth returning, as a bounded window around a hit. It needs PDF heading detection for the 75 non-EPUB sources, and it is a rebuild.
