...
- Tool answers are lean by default and the lean shape is a contract, not a style choice: an agent's context window is the scarcest resource in a research session. `tool_views.py` is the only projector from a service payload to an MCP tool answer, it keeps the evidence, the stable handles, the actionable state, and the counts, and it omits ranking internals, extraction diagnostics, storage paths, revision fingerprints, model identifiers, pipeline timings, and every empty, null, or harmless-default value. Never widen it to satisfy a diagnostic need.
- Only `--tool-detail full` (`RESEARCH_ULTRARAG_TOOL_DETAIL=full`) returns the complete service payload through a tool, and it exists for debugging rather than for agents. Diagnostic consumers must ask for it explicitly with `create_research_transport(..., tool_detail=FULL_TOOL_DETAIL)`; today those are the UI adapter, the terminal verifier, the bundle client, and the evaluation harness. A new consumer that reads internals must do the same instead of adding a field to the lean answer.
- Keep the lean answer and the full payload describing the same facts. A field that a lean answer omits must still exist, and mean the same thing, in the service payload the full-detail mode returns.
...
- `tool_views.py`: the lean agent-facing projection of every public tool payload, plus the full-detail debug path.
...
Each tool returns the lean projection from `tool_views.py` unless the server runs with `--tool-detail full`; the bullets below name what the service builds, which is what the full-detail mode returns.
...
- Disclose withholding instead of hiding it. Report reason codes, counts, and example chunk IDs in the full-detail search payload, and record corpus-level withheld counts and reasons in the generation build metrics that the full-detail `status` returns. The lean agent answer reports neither, and no rule may require a lean answer to carry a diagnostic.
