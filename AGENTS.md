...
- `ingest`: resumable, one call per `ingestion.work_budget_seconds` slice, and one build per project at a time. A call that arrives while another process holds the project lock is rejected promptly and names the resident build's phase and progress; it must never be queued for minutes, because the caller's client times out long before the work it is queued behind ends, and it then reports a failure the server never had. A checkpoint whose identity no longer matches the corpus is discarded, and the answer reports that as `superseded_build` with a message, so progress that restarts is explained instead of being mistaken for the caller's own error.
