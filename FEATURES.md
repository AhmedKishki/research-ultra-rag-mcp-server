...
| Minimum passage length (added here) | `retrieval.minimum_passage_words` drops a candidate whose cleaned text has fewer words than the setting. Chunks never span extraction units, so a short unit becomes a short chunk that matches a query about its own words: an index line, a heading, a caption, a copyright line. Off by default; the reference project sets 12. The full-detail payload reports the policy, the counts per rule, and the sources dropped. | A book's index is a list of the corpus's own words and is not evidence: 16.6% of the reference corpus's chunks are under 12 words, 2.7% of the passages its searches returned were, and dropping them put nine sources in an answer that had shown one of them. |
