...
| Core command line (added here) | `research-ultra-rag` resolves a project and calls the same `ResearchService` the server exposes, in the caller's own process: `init`, `status`, `ingest`, `search`, `sources`, `passage`, `include`, `exclude`, `metadata`, `config`, and `ui`. The UltraRAG gateway is opened on the first call that needs it, so the commands that only read local state never start one. Answers go through the same projector the tools use, and because settings resolve in process, `--set` reaches every operation. | A knowledge base can be created, built, searched, reviewed, and browsed with no MCP client, no agent, and no long-running server, and the whole surface is scriptable. |
