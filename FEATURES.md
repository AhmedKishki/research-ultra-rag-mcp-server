...
| Metadata filters (added here) | Narrow by category, keyword, or document, and see the whole partition inventory in `status`. | Keeps a search inside the part of the collection you care about. |
...
| Corpus partitions by category (added here) | `categories` requires every listed category and `categories_any` requires at least one, both resolved from reviewed metadata at query time; `status.categories` reports each category with its searchable source count. | One corpus can be searched as several parts — strands, chapter groups, sub-projects — and the partitioning is reviewable metadata, not an index-time decision. |
