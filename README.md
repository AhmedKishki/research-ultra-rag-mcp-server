...
| `retrieval.*` | Tune fusion, gating, and final selection: `rrf_k`, `bm25_weight`, `dense_weight`, `minimum_candidates`, `maximum_candidates`, `dense_minimum_cosine_similarity`, `rerank_max_candidates`, `rerank_window_multiple`, `rerank_window_floor`, `maximum_withheld_examples`, `source_diversity_penalty`. The fusion and gate values are part of the ranking policy recorded with a generation, so changing one makes the next ingestion a new generation; `source_diversity_penalty` is a runtime setting that reorders the candidates a search already ranked, so it applies to the next query and rebuilds nothing. |
