...
| `retrieval.*` | Tune fusion, gating, and final selection: `rrf_k`, `bm25_weight`, `dense_weight`, `minimum_candidates`, `maximum_candidates`, `dense_minimum_cosine_similarity`, `dense_relative_similarity_margin`, `minimum_passage_words`, `rerank_max_candidates`, `rerank_window_multiple`, `rerank_window_floor`, `maximum_withheld_examples`, `source_diversity_penalty`. The fusion and gate values are part of the ranking policy recorded with a generation, so changing one makes the next ingestion a new generation; `source_diversity_penalty`, `dense_relative_similarity_margin`, and `minimum_passage_words` are runtime settings that pick or reorder candidates for one query, so they apply at the next search and rebuild nothing. |
