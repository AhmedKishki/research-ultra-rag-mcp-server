...
`status` returns readiness, `stale`, the selected generation's `generation_id`, `created_at` and `chunk_count`, the source counts, `available_retrieval_methods`, `generation_upgrade_required` with `upgrade_reasons`, `metadata_overlay_active`, `metadata_pending_source_count`, `ingestion_progress`, what a prune would consider as `retained_generation_count` and `retained_generation_bytes`, and, when the generation is stale, `changes` — counts of added and modified sources, `removed_sources` naming the files that disappeared, and the review and exclusion flags. It inventories nothing: the retained-generation list, the category inventory, and the project inventory come back from `--tool-detail full`. `ingest` returns its `status`, `generation_changed`, and the document, chunk, vector, reuse, and discard counts. `set_source_inclusion` returns the decision, its reason, `effective_immediately`, and whether the next ingestion should rebuild without the source.
