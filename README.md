...
- **runtime** — thread counts, batch sizes, the work budget, log level, tool detail, where the shared model cache lives, and the source-diversity penalty, which reorders the candidates a search already ranked. These shape this process and its answers but cannot change an artifact.
...
| `retrieval.*` | Tune fusion, gating, and final selection: `rrf_k`, `bm25_weight`, `dense_weight`, `minimum_candidates`, `maximum_candidates`, `dense_minimum_cosine_similarity`, `dense_relative_similarity_margin`, `minimum_passage_words`, `minimum_passage_token_fraction`, `rerank_max_candidates`, `rerank_window_multiple`, `rerank_window_floor`, `maximum_withheld_examples`, `source_diversity_penalty`. The fusion and gate values are part of the ranking policy recorded with a generation, so changing one makes the next ingestion a new generation; `source_diversity_penalty`, `dense_relative_similarity_margin`, `minimum_passage_words`, and `minimum_passage_token_fraction` are runtime settings that pick or reorder candidates for one query, so they apply at the next search and rebuild nothing. |
| `chunking.size`, `chunking.overlap` | Chunk length and overlap in GPT-2 tokens: size 50–384, overlap below the size. 384 is the ceiling because the embedding model reads at most 512 tokens, so a chunk has to leave room for the contextual header and stay inside it; the ingestion audit counts every chunk whose vector covers only a prefix, and 0.1% of the reference corpus's chunks are truncated at 384. Identity settings, recorded with the generation: changing either re-chunks and re-embeds. |
...
| `ingestion.work_budget_seconds` | Soft time budget for one `ingest` call before it returns a checkpointed `in_progress` result; up to an hour, so a caller that cannot repeat identical calls can set one call to carry the whole build. Keep it below the client's own request timeout. |
...
- An MCP client that keeps one settings file for every window gives each window's server the same `--ui-port`. Only the first server claims it; the others keep serving tools with `ui_ready: false` and `ui_error` naming the taken port, and every window still carries its own server, gateway and UI stack. With more than one window or project open, a per-project launcher that starts `research-ultra-rag-ui` with that project's `--project-root`, `--runtime-root`, and `--port` stays predictable in a way a shared `--ui-port` cannot. A server also ends itself when the process that started it is gone, so a build cannot outlive the window that asked for it: every child this project spawns is told its owner, and `RESEARCH_ULTRARAG_OWNER_PID` is the variable a client that spawns the server itself can set to name it.
...
| `set_source_metadata` | Saves reviewed bibliographic metadata for one source — title, authors, year, DOI, language, categories, keywords, project — in the review-state file. `language` takes ISO 639 codes, and a code BM25 has no stopword list for is accepted because the field describes the source rather than the index. An empty review clears the entry so automatic metadata applies again. Applies at the next read and never changes the source file. |
...
**Chunking.** UltraRAG splits units into GPT-2 token chunks of the configured size. Several units share one chunker call, which is a throughput optimisation only: each unit still gets its own durable output file and its own place to resume from. A unit is never merged with another, so a chunk is at most one extraction unit whatever the configured size is, and short units stay short: neither the chunk size nor the overlap removes a fragment, which is why the fragment rule is `retrieval.minimum_passage_token_fraction` rather than a chunking hint.
...
**Relevance gates.** A dense candidate must clear a cosine floor, and a lexical one must share a non-stopword token with the query, so a search can legitimately return fewer passages than asked for, including none. A floor is absolute, and a one-word query's whole candidate list can sit in a narrow band under it: on the reference corpus `waste` spanned 0.681 to 0.742 across fifty candidates, where the floor kept six of them and four sources. `retrieval.dense_relative_similarity_margin` then admits that band, but only when at least one candidate cleared the floor; when none did, nothing is admitted, because a query the corpus cannot support has to abstain rather than return its least-bad passage. A candidate also has to be long enough to cite, measured two ways: `retrieval.minimum_passage_words` counts words, and `retrieval.minimum_passage_token_fraction` counts tokens as a fraction of the generation's recorded `chunking.size`, which is the unit the chunker counted in. Chunks never span extraction units, so a short unit becomes a short chunk — a five-word index line, a heading, a caption, a copyright line — that matches a query about its own words while carrying no prose, and a length rule is the only gate that can see it. The token floor is counted over the returned text with the generation's recorded tokenizer, so a contextual header cannot make a fragment look long. The full-detail payload reports the query's best cosine similarity, how many candidates each rule admitted or dropped, and which sources it dropped, so a thin answer reads as thinned rather than silent.

**Final selection.** Fusion and the reranker produce one ranked list, and the answer is its first `top_k`. `retrieval.source_diversity_penalty` charges a candidate a share of its normalized relevance for every candidate already taken from the same source, so ten results come from ten different sources instead of one well-covered document. It can only reorder that list: never add, drop, or rescore a candidate, and `0` returns the ranked order exactly. A BM25 or dense ranking without reranking carries no score to charge a repeat against, so it also keeps its own order. On the reference corpus it changed the target's rank in one of thirty judged queries while the mean number of distinct sources in a top-10 went from 3.9 to 7.4.

**The sidecar.** A compact SQLite file inside each generation stores IDs, content hashes, vector ordinals, byte offsets, and one precomputed retrieval verdict per chunk. The verdict records whether a chunk is structurally unusable (`corrupt-text`, no readable content, extraction artifact) so a query can reject it without re-reading its text. It is derived metadata, never a copy of your text.
