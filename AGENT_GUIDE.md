...
4. If `stale=true`, explain the reported source or inclusion changes. The prior generation remains searchable. Ask whether to re-ingest. A true `metadata_overlay_active` is not staleness: reviewed metadata is already effective without rebuilding the generation. A non-zero `metadata_pending_source_count` counts reviewed sources outside the selected generation; their reviews take effect after those sources are included and ingested.
...
- `source_id`: the project-scoped handle for metadata and inclusion edits. It survives content changes but changes when the source is renamed or moved.
