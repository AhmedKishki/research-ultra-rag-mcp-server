...
- `rank`: this passage's position in this answer. It is not a quality score and is not comparable between searches.
...
- In the reference view, read `reference_groups`: each entry names one reference and the passages selected from it, with each passage keeping its `rank`. `passages_per_reference` caps each source, and `top_k` remains the total passage budget.
